...
f = Dataset('files/fix_global_v6_fun_pft_mean.nc','r')
...
pnonmyc_ori = f.variables['NNONMYC'][:]
pactive_ori = f.variables['NACTIVE'][:]
pretrans_ori = f.variables['NRETRANS'][:]
pfix_ori = f.variables['NFIX'][:]
...
pfix_arr_time= np.zeros((len(time),len(pft)))
...
for j in xrange(len(time)):
  for i in xrange(len(pft)):
   ...
   pfix_arr_time[j,i] = (np.mean(np.ma.average(pfix[j,i,:,:], \
                    axis=0,weights=weights)))

   pfix_arr.append(np.mean(np.ma.average(pfix[j,i,:,:], \
                     axis=0,weights=weights)))

   pfix_ave.append(np.ma.average(pfix[j,i,:,:], \
                    axis=0,weights=weights))

   pfix_var.append(np.ma.average((pfix[j,i,:,:]-pfix_ave[i])**2,axis=0,weights=weights))
...
pfix_arr_glob = np.zeros((len(time),6))
...
for j in xrange(len(time)): 
  ...
  pfix_arr_glob[j] = [np.sum([pfix_arr[j,6],pfix_arr[j,7],pfix_arr[j,8]]), \
                    np.sum([pfix_arr[j,12],pfix_arr[j,13],pfix_arr[j,14], \
                                          pfix_arr[j,15],pfix_arr[j,16]]), \
                    np.sum([pfix_arr[j,1],pfix_arr[j,2]]), \
                    np.sum([pfix_arr[j,3]]), \
                    np.sum([pfix_arr[j,4],pfix_arr[j,5]]), \
                    np.sum([pfix_arr[j,10],pfix_arr[j,11]])]
...
for i, pft in enumerate(my_pfts):
   	...
   	fig = plt.figure(figsize=(48, 24)) 
        ...
        plt.stackplot(time[:],np.array(pnonmyc_arr_glob[i][:])*(const),np.array(pactive_arr_glob[i][:])*(const),np.array(pretrans_arr_glob[i][:]/1.)*(const),np.array(pfix_arr_glob[i][:]/1.)*(const),edgecolor='k',linewidth=3.3,colors=['b','r','darkorange','orchid'],labels=['Direct root uptake','Mycorrhizal uptake','Retranslocation','Symbiotic fixation']) 
	...
	plt.xlabel(r'Month')
	plt.ylabel(r'Plant N uptake (g N m$^{-2}$ month$^{-1}$)' )
        #plt.ylabel('' +long_name+' ('+unit[0:2]+' m$^2$ yr$^{-1}$)' )

        ttl=plt.title(r' %s' % (pft) ,fontsize = 84)
        ttl.set_position([.5, 1.05])

        plt.savefig('figures/Nitrogen_fun/NUPTAKE_monthly_mean_%s.png' % (pft),bbox_inches="tight")
...
ptotal_std_glob = np.sqrt(np.array(pnonmyc_std_glob)**2+np.array(pactive_std_glob)**2+(np.array(pretrans_std_glob)/100.)**2)
...
p3 = plt.bar(ind, np.array(pretrans_arr_glob)/100., width,color = 'darkorange',edgecolor='k',linewidth=3.3, bottom=pactive_arr_glob,yerr=ptotal_std_glob, error_kw=dict(lw=5, capsize =5, capthick=3))
plt.ylabel(r'Carbon use (g C m$^{-2}$ yr$^{-1}$)')
plt.xticks(ind,('Evergreen\n broadleaf','Evergreen\n needleleaf','Grassland','Decidous\n broadleaf','Shrubland','Deciduos\n needleleaf'))
plt.legend((p1[0],p2[0],p3[0]),(r'Direct root uptake','Mycorrhizal uptake','Retranslocation ($\div$ 100)'))
plt.ticklabel_format(axis="y",style="sci",scilimits=(0,0))
plt.savefig('COST_PUPTAKE_zonal_pft.png',bbox_inches="tight")
#plt.show()
