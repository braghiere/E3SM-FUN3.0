...
leafp = f.variables['LEAFN'][:]
leafc = f.variables['LEAFC'][:]

retransp_to_ppool = f.variables['RETRANSN_TO_NPOOL'][:]
free_retransp_to_ppool = f.variables['FREE_RETRANSN_TO_NPOOL'][:]
...
#pactive = np.ma.masked_where(pactive < 10e-20, pactive)
...
for i in xrange(1):
  ...
  #levels=[-0.0051251*max_val,0.,0.0051251*max_val,0.01251*max_val,0.0625*max_val,.1*max_val,.2*max_val,.3*max_val,.4*max_val,.5*max_val,.6*max_val,.7*max_val,.75*max_val,.8*max_val,.9*max_val,1.*max_val]
   ...
   plt.savefig('figures/Nitrogen_fun/NRETRANS_ratio.png',bbox_inches="tight")
