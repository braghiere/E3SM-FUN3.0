...
for i in xrange(1):
      ...
      gpp_mte_ori_mean_puptake.append(np.mean(puptake,axis = 1))
      ...
      gpp_mte_ori_mean_nfix.append(np.mean(nfix,axis = 1))
...
for i, pft in enumerate(my_pfts):
   	...
   	fig = plt.figure(figsize=(48, 24)) 
        ...
        const = 100./np.array(gpp_mte_ori_mean_puptake[0][:])
        #print(np.array(gpp_mte_ori_mean_puptake[0][:]))

        plt.stackplot(np.floor(((np.asarray(x) + 1)*res_lat) -90.),np.array(gpp_mte_ori_mean_pnonmyc[0][:])*(const),np.array(gpp_mte_ori_mean_pam[0][:])*(const),np.array(gpp_mte_ori_mean_pecm[0][:])*(const),np.array(gpp_mte_ori_mean_pretrans[0][:])*(const),np.array(gpp_mte_ori_mean_nfix[0][:])*(const),edgecolor='k',linewidth=3.3,colors=['chartreuse','b','r','yellow','orchid']) 
 
        #plt.stackplot(np.floor(((np.asarray(x) + 1)*res_lat) -90.),np.array(gpp_mte_ori_mean_pnonmyc[0][:])*(const),np.array(gpp_mte_ori_mean_pam[0][:])*(const),np.array(gpp_mte_ori_mean_pecm[0][:])*(const),np.array(gpp_mte_ori_mean_pretrans[0][:])*(const),edgecolor='k',linewidth=3.3,colors=['chartreuse','b','r','yellow']) 
	...
	plt.ylabel(r'Total annual N uptake (%)' )
        #plt.ylabel('' +long_name+' ('+unit[0:2]+' m$^2$ yr$^{-1}$)' )

        #ttl=plt.title(r'ZONAL MEAN - %s' % (pft) ,fontsize = 84)
        #ttl.set_position([.5, 1.05])

        plt.savefig('figures/Nitrogen_fun/NUPTAKE_zonal_mean_ratio.png',bbox_inches="tight")
