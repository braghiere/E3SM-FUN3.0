...
for i, pft in enumerate(my_pfts):
   	...
   	fig = plt.figure(figsize=(48, 24)) 
        ...
        plt.stackplot(np.floor(((np.asarray(x) + 1)*res_lat) -90.),np.array(gpp_mte_ori_mean_pnonmyc[0][:])*(const),np.array(gpp_mte_ori_mean_pam[0][:])*(const),np.array(gpp_mte_ori_mean_pecm[0][:])*(const),np.array(gpp_mte_ori_mean_pretrans[0][:])*(const),np.array(gpp_mte_ori_mean[0][:])*(const),edgecolor='k',linewidth=3.3,colors=['chartreuse','b','r','yellow','orchid'],labels=['Direct root uptake','AM root uptake','ECM root uptake','Retranslocation','Symbiotic fixation']) 
	...
	plt.ylabel(r'Total annual N uptake (Tg N yr$^{-1}$)' )
        #plt.ylabel('' +long_name+' ('+unit[0:2]+' m$^2$ yr$^{-1}$)' )

        #ttl=plt.title(r'ZONAL MEAN - %s' % (pft) ,fontsize = 84)
        #ttl.set_position([.5, 1.05])

        plt.savefig('figures/Nitrogen_fun/NUPTAKE_zonal_mean.png',bbox_inches="tight")
