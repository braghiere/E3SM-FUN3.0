...
lon = f.variables['lon'][:]
gpp = f.variables['GPP'][:]
pactive = f.variables['PUPTAKE_NPP_FRACTION'][:]
...
mean_val = np.mean(v)
max_val = 15.
print('Maximum frac =',max_val)
print('Mean frac =',mean_val)
...
for i in range(1):
  ...
  #levels=[-0.0051251*max_val,0.,0.0051251*max_val,0.01251*max_val,0.0625*max_val,.1*max_val,.2*max_val,.3*max_val,.4*max_val,.5*max_val,.6*max_val,.7*max_val,.75*max_val,.8*max_val,.9*max_val,1.*max_val]
   ...
   plt.title(r'Phosphorus', fontname='Times', fontsize=92*1.5,pad=26)
   cbar.ax.set_xlabel('C use ratio (%)', rotation=0,color='black', size=78*1.5)
   ...
   plt.savefig('figures/PUPTAKE_NPP_ratio.png',bbox_inches="tight")
