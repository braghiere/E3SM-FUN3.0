...
lon = f.variables['lon'][:]
gpp = f.variables['GPP'][:]
pactive = f.variables['COST_PRETRANS'][:]
...
print('Cost_pretrans',np.nanmean(v),'gP/gC')
...
for i in range(1):
   ...
   cbar.ax.set_xlabel('g P g C$^{-1}$', rotation=0,color='black', size=78, fontname='Times')
   ...
   plt.title(r'C cost of P retranslocation', fontname='Times', fontsize=92,pad=26)
   cbar.ax.tick_params(labelsize='xx-large')
   #plt.savefig('em_steindinger_tot.pdf',bbox_inches="tight",dpi=300)
   plt.savefig('figures/COST_PRETRANS.png',bbox_inches="tight")
