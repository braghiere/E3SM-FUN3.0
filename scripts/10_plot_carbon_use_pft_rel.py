...
cost_nonmyc_ori = f.variables['COST_NNONMYC'][:]
cost_active_ori = f.variables['COST_NACTIVE'][:]
cost_retrans_ori = f.variables['COST_NRETRANS'][:]

#cost_active_ori[cost_active_ori ==  0]=np.nan
#cost_nonmyc_ori[cost_nonmyc_ori == 0]=np.nan
#cost_retrans_ori[cost_retrans_ori == 0]=np.nan
...
pnonmyc_ori = pnonmyc_ori/cost_nonmyc_ori
pactive_ori = pactive_ori/cost_active_ori
pretrans_ori = pretrans_ori/cost_retrans_ori
...
pactive_ori[pactive_ori >= 1.e+2]=0.
pactive_ori[pactive_ori == 1]=0.
#pactive_ori[pactive_ori == 0]=np.nan

pnonmyc_ori[pnonmyc_ori >= 1.e+2]=0.
pnonmyc_ori[pnonmyc_ori == 1]=0.
#pnonmyc_ori[pnonmyc_ori == 0]=np.nan

pretrans_ori[pretrans_ori >= 1.e+2]=0.
...
plt.savefig('figures/10_test.png')
plt.close()
#sys.exit()
...
p1 = plt.bar(ind, 100.*np.array(pactive_arr_glob)/tot, width, color = '#edf8b1',edgecolor='k',linewidth=3.3)

p2 = plt.bar(ind, 100.*np.array(pnonmyc_arr_glob)/tot, width, color = '7fcdbb',edgecolor='k',linewidth=3.3, bottom=100.*np.array(pactive_arr_glob)/tot)


#p3 = plt.bar(ind, 100.*np.array(pretrans_arr_glob)/tot, width,color = 'darkorange',edgecolor='k',linewidth=3.3,yerr=100.*ptotal_std_glob/tot, error_kw=dict(lw=5, capsize =5, capthick=3), bottom=100.*(np.array(pactive_arr_glob)+np.array(pnonmyc_arr_glob))/tot)
p3 = plt.bar(ind, 100.*np.array(pretrans_arr_glob)/tot, width,color = '#2c7fb8',edgecolor='k',linewidth=3.3, bottom=100.*(np.array(pactive_arr_glob)+np.array(pnonmyc_arr_glob))/tot)

plt.ylabel(r'Carbon use (%)')
#plt.ylabel(r'Carbon use (g C m$^{-2}$ yr$^{-1}$)')
...
plt.ylim(0.,100.)
plt.savefig('figures/COST_NUPTAKE_zonal_pft_rel.png',bbox_inches="tight")
#plt.show()
