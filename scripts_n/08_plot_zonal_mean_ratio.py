...
for i, pft in enumerate(my_pfts):
   	...
   	fig = plt.figure(figsize=(48, 24)) 
        ...
        plt.stackplot(np.floor(((np.asarray(x) + 1)*res_lat) -90.),np.array(gpp_mte_ori_mean_pnonmyc[0][:])*(const),np.array(gpp_mte_ori_mean_pam[0][:])*(const),np.array(gpp_mte_ori_mean_pecm[0][:])*(const),np.array(gpp_mte_ori_mean_nfix[0][:])*(const),np.array(gpp_mte_ori_mean_pretrans[0][:])*(const),edgecolor='k',linewidth=3.3,colors=['chartreuse','b','r','orchid','yellow']) 
	...
	plt.ylabel(r'Total annual N uptake (%)' )
        ...
        plt.savefig('figures/Nitrogen/NUPTAKE_zonal_mean_ratio.png',bbox_inches="tight")
