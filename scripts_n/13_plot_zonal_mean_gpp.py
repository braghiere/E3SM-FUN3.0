...
npp_elm = f1.variables['GPP'][:]
npp_elm,lon1 = shiftgrid(180., npp_elm, lon1, start=False)

lon2 = f2.variables['lon'][:]
npp_elm_fun = f2.variables['GPP'][:]
npp_elm_fun,lon2 = shiftgrid(180., npp_elm_fun, lon2, start=False)

lon_flux = fluxcom.variables['lon'][:]
lat_flux = fluxcom.variables['lat'][:]
gpp_flux = fluxcom.variables['GPP'][:]
...
print 'Global GPP_FUNP  =', mean_corr, '+-', mean_corr-h_dn_corr, 'PgCyr-1'
...
print 'Global GPP =', mean_corr, '+-', mean_corr-h_dn_corr, 'PgCyr-1'
...
print 'Global GPP_FUN =', mean_corr, '+-', mean_corr-h_dn_corr, 'PgCyr-1'
...
lat_l = lat_flux/dtor
lat_u = (lat_flux + res_lat)/dtor
weights = (radius*radius*(lon_u - lon_l)*(np.sin(lat_u)-np.sin(lat_l)))

#GPP FLUXCOM
gpp_flux = np.array(gpp_flux)
gpp_flux[gpp_flux==-9999.]=np.nan

gpp_flux=np.ma.masked_array(gpp_flux,np.isnan(gpp_flux))
average=np.ma.average(gpp_flux,axis=0,weights=weights)
variance=np.ma.average((gpp_flux-average)**2,axis=0,weights=weights)

mean_corr = np.mean(np.ma.average(gpp_flux,axis=0,weights=weights))*(365)*148847000*(1000*1000)/(10**15)

std_corr = np.mean(np.sqrt(variance))*(365)*148847000*(1000*1000)/(10**15)

h_dn_corr, h_up_corr = stats.norm.interval(0.66, loc=mean_corr, scale= std_corr/np.sqrt(len(np.ma.array(gpp_flux))))

print 'Global GPP_FLUXCOM =', mean_corr, '+-', mean_corr-h_dn_corr, 'PgCyr-1'
...
for i, pft in enumerate(my_pfts):
	...
	plt.ylabel(r'GPP (g C m$^{-2}$ yr$^{-1}$)' )
        #plt.ylabel('' +long_name+' ('+unit[0:2]+' m$^2$ yr$^{-1}$)' )

        #ttl=plt.title(r'ZONAL MEAN - %s' % (pft) ,fontsize = 84)
        #ttl.set_position([.5, 1.05])

        plt.savefig('figures/Nitrogen/GPP_zonal_mean.png',bbox_inches="tight")
