...
#pnonmyc_ori = f.variables['PNONMYC'][:]/f.variables['COST_PNONMYC'][:]
#pactive_ori = f.variables['PACTIVE'][:]/f.variables['COST_PACTIVE'][:]
#pretrans_ori = f.variables['PRETRANS'][:]/f.variables['COST_PRETRANS'][:]
...
nfix_ori = f.variables['NFIX'][:]/f.variables['COST_NFIX'][:]

pnonmyc_ori = f.variables['NPP_NNONMYC'][:]
pactive_ori = f.variables['NPP_NACTIVE'][:]
pretrans_ori = f.variables['NPP_NRETRANS'][:]
nfix_ori = f.variables['NPP_NFIX'][:]
...
lon = f.variables['lon'][:]
nfix_ori,lon = shiftgrid(180., nfix_ori, lon, start=False)


pactive_ori[pactive_ori >= 1.e+30]=0.
pactive_ori[pactive_ori == 1]=0.
#pactive_ori[pactive_ori == 0]=np.nan

pnonmyc_ori[pnonmyc_ori >= 1.e+30]=0.
pnonmyc_ori[pnonmyc_ori == 1]=0.
#pnonmyc_ori[pnonmyc_ori == 0]=np.nan

pretrans_ori[pretrans_ori >= 1.e+30]=0.
pretrans_ori[pretrans_ori == 1]=0.
#pretrans_ori[pretrans_ori == 0]=np.nan

nfix_ori[nfix_ori >= 1.e+30]=0.
nfix_ori[nfix_ori == 1]=0.
#nfix_ori[nfix_ori == 0]=np.nan


#for i in xrange(16):
#   plt.imshow(nfix_ori[i,:,:])
#   plt.show()
#sys.exit()
...
print('Nfix',np.sum(npp_nfix_tot))
print('Total',np.sum(npp_pretrans_tot+npp_pnonmyc_tot+npp_pactive_tot+npp_nfix_tot))
#sys.exit()
...
nfix_mean = np.nanmean(np.nanmean(nfix,axis=1),axis=1)
...
nfix_std = np.nanstd(np.nanstd(nfix,axis=1),axis=1)

total_std = np.sqrt(pretrans_std**2 + pnonmyc_std**2 + pactive_std**2 + nfix_std**2)

#plt.plot(pretrans_mean,label='pretrans')
#plt.plot(pactive_mean,label='pactive')
#plt.plot(pnonmyc_mean,label='pnonmyc')
#plt.plot(nfix_mean,label='nfix')
#plt.legend()
...
p4 = plt.bar(ind, np.array(nfix_mean), width,color = 'pink',edgecolor='k',linewidth=3.3,bottom=np.array(pactive_mean)+np.array(pnonmyc_mean)+np.array(pretrans_mean))



plt.ylabel(r'Carbon use (g C m$^{-2}$ yr$^{-1}$)')
#plt.ylabel(r'Carbon cost (g N g C$^{-1}$)')
#plt.xticks(ind,('Evergreen\n broadleaf','Evergreen\n needleleaf','Decidous\n broadleaf','Grassland','Shrubland','Deciduos\n needleleaf'))
plt.xticks(ind,pftname_fig,rotation=45)
plt.legend((p1[0],p2[0],p3[0],p4[0]),('Mycorrhizal uptake','Direct root uptake','Retranslocation','N fixation'))
#($\div$ 100)
plt.ticklabel_format(axis="y",style="sci",scilimits=(0,0))
plt.savefig('figures/Nitrogen/carbon_use_by_pft_all.png')
plt.close()
#sys.exit()
...
for i in xrange(len(pft)):
 ...
 #NFIX
 nfix_arr.append(np.mean(np.ma.average(nfix[i,:,:], \
                     axis=0,weights=weights)))

 nfix_ave.append(np.ma.average(nfix[i,:,:], \
                    axis=0,weights=weights))

 nfix_var.append(np.ma.average((nfix[i,:,:]-nfix_ave[i])**2,axis=0,weights=weights))

 nfix_std_corr.append(np.mean(np.sqrt(nfix_var[i])))

 nfix_h_dn_corr.append(stats.norm.interval(0.66, loc=nfix_arr[i], scale= nfix_std_corr[i]/np.sqrt(len(np.ma.array(nfix[i,:,:]))))[0])
...
nfix_h_dn_corr = np.array(nfix_arr) - np.array(nfix_h_dn_corr)

nfix_arr_glob = [np.nansum([nfix_arr[6],nfix_arr[7],nfix_arr[8]]), \
                    np.nansum([nfix_arr[12],nfix_arr[13], \
                                          nfix_arr[15]]), \
                    np.nansum([nfix_arr[1],nfix_arr[2]]), \
                    np.nansum([nfix_arr[3]]), \
                    np.nansum([nfix_arr[4],nfix_arr[5]]), \
                    np.nansum([nfix_arr[10],nfix_arr[11]])]

nfix_std_glob = [np.sqrt(np.nansum([nfix_h_dn_corr[6]**2,nfix_h_dn_corr[7]**2,nfix_h_dn_corr[8]**2])), \
                    np.sqrt(np.nansum([nfix_h_dn_corr[12]**2,nfix_h_dn_corr[13]**2, \
                                          nfix_h_dn_corr[15]**2])), \
                    np.sqrt(np.nansum([nfix_h_dn_corr[1]**2,nfix_h_dn_corr[2]**2])), \
                    np.sqrt(np.sum([nfix_h_dn_corr[3]**2])), \
                    np.sqrt(np.nansum([nfix_h_dn_corr[4]**2,nfix_h_dn_corr[5]**2])), \
                    np.sqrt(np.nansum([nfix_h_dn_corr[10]**2,nfix_h_dn_corr[11]**2]))]
...
nfix_arr_glob = [nfix_arr_glob[1],nfix_arr_glob[5], \
                     nfix_arr_glob[0],nfix_arr_glob[4], \
                     nfix_arr_glob[2],nfix_arr_glob[3]]

nfix_std_glob = [nfix_std_glob[1],nfix_std_glob[5], \
                     nfix_std_glob[0],nfix_std_glob[4], \
                     nfix_std_glob[2],nfix_std_glob[3]]


ptotal_std_glob = np.sqrt(np.array(pnonmyc_std_glob)**2+np.array(pactive_std_glob)**2+(np.array(pretrans_std_glob))**2+(np.array(nfix_std_glob))**2)

print('Grassland','Shrubland','Decidous\n broadleaf','Evergreen\n broadleaf','Evergreen\n needleleaf','Deciduos\n needleleaf')
tot = np.array(pretrans_arr_glob) + np.array(pnonmyc_arr_glob) + np.array(pactive_arr_glob)+np.array(nfix_arr_glob)
...
print('nfix',nfix_arr_glob,nfix_arr_glob/tot)
#sys.exit()
...
p3 = plt.bar(ind, np.array(pretrans_arr_glob), width,color = 'darkorange',edgecolor='k',linewidth=3.3, bottom=np.array(pactive_arr_glob)+np.array(pnonmyc_arr_glob))

p4 = plt.bar(ind, np.array(nfix_arr_glob), width,color = 'pink',edgecolor='k',linewidth=3.3,yerr=ptotal_std_glob, error_kw=dict(lw=5, capsize =5, capthick=3), bottom=np.array(pactive_arr_glob)+np.array(pnonmyc_arr_glob)+np.array(pretrans_arr_glob))


#plt.ylabel(r'Carbon cost (g N g C$^{-1}$)')
plt.ylabel(r'Carbon use (g C m$^{-2}$ yr$^{-1}$)')
plt.xticks(ind,('Grassland','Shrubland','Decidous\n broadleaf','Evergreen\n broadleaf','Evergreen\n needleleaf','Deciduos\n needleleaf'), rotation=45)
plt.legend((p1[0],p2[0],p3[0],p4[0]),('Mycorrhizal uptake','Direct root uptake','Retranslocation','N fixation'),loc='best')
#($\div$ 100)
#plt.ticklabel_format(axis="y",style="sci",scilimits=(0,0))
plt.ticklabel_format(axis="y")
plt.savefig('figures/Nitrogen/COST_NUPTAKE_zonal_pft.png',bbox_inches="tight")
#plt.show()
