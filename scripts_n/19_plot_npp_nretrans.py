...
lon = f.variables['lon'][:]
gpp = f.variables['GPP'][:]
pactive = f.variables['NPP_NRETRANS'][:]
...
print 'Global npp_nretrans =', mean_corr, '+-', mean_corr-h_dn_corr, 'PgCyr-1'

print('npp_nretrans',np.nanmean(v),'gCm-2yr-1')
...
for i in xrange(1):
   ...
   plt.title(r'NPP spent on N retranslocation', fontname='Times', fontsize=92,pad=26)
   cbar.ax.tick_params(labelsize='xx-large')
   #plt.savefig('em_steindinger_tot.pdf',bbox_inches="tight",dpi=300)
   plt.savefig('figures/Nitrogen/NPP_NRETRANS.png',bbox_inches="tight")
