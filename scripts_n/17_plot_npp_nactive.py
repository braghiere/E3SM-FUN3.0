...
lon = f.variables['lon'][:]
gpp = f.variables['GPP'][:]
pactive = f.variables['NPP_NUPTAKE'][:]
#pactive = f.variables['NACTIVE'][:]/f.variables['COST_NACTIVE'][:]
...
print('npp_pactive',np.nanmean(v),'gCm-2yr-1')
...
for i in xrange(1):
   ...
   plt.title(r'NPP spent on N uptake', fontname='Times', fontsize=92,pad=26)
   cbar.ax.tick_params(labelsize='xx-large')
   #plt.savefig('em_steindinger_tot.pdf',bbox_inches="tight",dpi=300)
   plt.savefig('figures/Nitrogen/NPP_NUPTAKE.png',bbox_inches="tight")
